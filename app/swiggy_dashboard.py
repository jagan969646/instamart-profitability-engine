...
@st.cache_data
def load_data():
    file_path = 'swiggy_simulated_data.csv'
    if not os.path.exists(file_path):
        # Create a dummy dataframe so the app doesn't crash if file is missing
        st.error(f"Critical Error: '{file_path}' not found in repository!")
        return pd.DataFrame(columns=['order_time', 'order_value', 'zone', 'weather', 'delivery_cost', 'discount', 'category', 'freshness_hrs_left'])
...
if not df.empty:
    ...
    # Using 'Sorted' to keep the UI clean
    zones = sorted(df['zone'].unique())
    selected_zones = st.sidebar.multiselect("Select Target Zones", options=zones, default=zones[:3])
    ...
    with kpi1: st.markdown(f'<div class="kpi-metric">₹{total_gov/1e6:.2f}M<br><span class="kpi-label">Total GOV</span></div>', unsafe_allow_html=True)
    with kpi2: st.markdown(f'<div class="kpi-metric">₹{avg_cm:.2f}<br><span class="kpi-label">Avg Profit/Order</span></div>', unsafe_allow_html=True)
    with kpi3: st.markdown(f'<div class="kpi-metric">{burn_rate:.1f}%<br><span class="kpi-label">Burn Rate</span></div>', unsafe_allow_html=True)
    with kpi4: st.markdown(f'<div class="kpi-metric">{len(filtered_df):,}<br><span class="kpi-label">Orders Modeled</span></div>', unsafe_allow_html=True)
    ...
    with row2_col1:
        st.subheader("📍 CM Analysis by Zone")
        zone_data = filtered_df.groupby('zone')['net_profit'].mean().reset_index().sort_values('net_profit')
        fig_zone = px.bar(zone_data, x='net_profit', y='zone', orientation='h', 
                          color='net_profit', color_continuous_scale='RdYlGn', template="simple_white")
    ...
    st.divider()
    st.subheader("🍏 Inventory Salvage Engine")
    ...
    with col_a2:
        st.warning(f"**Action Required:** {len(high_risk)} units have <12 hours shelf life.")
        if st.button("🚀 Trigger 'Flash Deals'"):
            st.success(f"Push notifications sent to users in {', '.join(selected_zones)}!")

st.markdown("---")
st.caption("By Jagadeesh.N | Instamart Profitability Engine 2026")
