...
with kpi1:
    st.markdown(f'<div class="kpi-metric">₹{total_gov/1e6:.2f}M<br><span class="kpi-label">Total GOV</span></div>', unsafe_allow_html=True)
with kpi2:
...
with row2_col1:
    ...
    zone_data = (
        filtered_df.groupby('zone')['net_profit']
        .mean()
        .reset_index()
...
with col_a1:
    fig_decay = px.histogram(perishables, x='freshness_hrs_left', nbins=30)
